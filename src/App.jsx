...
function DashboardRouter() {
  ...
  console.log('🔀 Rol detectado:', userProfile?.rol);

  if (userProfile?.rol === 'Coordinador') {
    console.log('🔀 Renderizando DashboardCoordinador');
...
function ProtectedLayout({ children }) {
  ...
  console.log('🛡️ ProtectedLayout - user:', user?.id, 'loading:', loading);
  ...
  console.log('🛡️ Renderizando Layout con children');
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#ecf0f1' }}>
      ...
      <div style={{ display: 'flex' }}>
        <Sidebar />
        <main style={{ flex: 1, padding: '2rem', minHeight: 'calc(100vh - 82px)' }}>
          {children}
        </main>
      </div>
    </div>
